...
- `rateLimit.ts`: In-memory request throttling for global and auth-specific routes.
...
2. Rate limiting (`rateLimit`) before route handlers.
3. Route handlers and validation.
4. Error handling (`errorHandler`) as the terminal middleware.
